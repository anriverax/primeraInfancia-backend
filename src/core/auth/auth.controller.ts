...
import { ChangePasswdCommand } from "./cqrs/commands/changePasswd/change-passwd.command";
import { RegisterUserCommand } from "./cqrs/commands/register/register-user.command";
import { FindUniqueUserQuery } from "./cqrs/queries/find-user-by-id.handler";
...
export class AuthController {
  ...
  async changePassword(@Req() req: Request, @Body() data: ChangePasswdDto): Promise<NestResponse<void>> {
    ...
    const isTheSamePassword = await this.authDomainService.passwordHashingService.comparePasswords(
      data.value3,
      isExist.passwd
    );
    ...
    return this.commandBus.execute(new ChangePasswdCommand(req));
