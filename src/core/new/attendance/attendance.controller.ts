...
export class AttendanceController {
  ...
  @AuthRequired()
  @Get("me/events")
  async listEvents(): Promise<EventHandlerResponse[]> {
  ...
  async listSupport(
    ...
    @Query("isMentor") isMentor: string
  ) {
    ...
    if (RoleType.USER_TECNICO_APOYO === role) {
      userRole = Boolean(isMentor) ? RoleType.USER_MENTOR : RoleType.USER_FORMADOR;
    }
  }
  @AuthRequired()
  @Get("me/teachers")
  async listTeachers(
    ...
    const result = await this.queryBus.execute(new GetTeacherAssignmentsByUserIdQuery(userId, userRole));
