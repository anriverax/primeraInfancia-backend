...
import { GetAllPermissionQuery } from "../application/queries/getAll-permission.query";
import { LoginCommand } from "../application/commands/login.command";
import { CreateUserCommand } from "../application/commands/create-user.command";
import { ChangePasswdCommand } from "../application/commands/change-passwd.command";
import { AuthDomainService } from "../domain/services/authDomain.service";
import { GetRefreshTokenQuery } from "../application/queries/refresh-token.query";
...
export class AuthController {
  ...
  async login(@Body() data: LoginDto): Promise<NestResponse<ILoginResponse>> {
    const email = data.value1.toLowerCase().trim();
    const password = data.value2.trim();
  ...
  async refreshToken(@Req() req: Request): Promise<NestResponse<ILoginResponse>> {
    ...
    const data = await this.queryBus.execute(new GetRefreshTokenQuery(req["token"], email));
  ...
  async changePassword(@Req() req: Request, @Body() data: ChangePasswdDto): Promise<NestResponse<void>> {
    ...
    await this.commandBus.execute(new ChangePasswdCommand(id, email, data));
