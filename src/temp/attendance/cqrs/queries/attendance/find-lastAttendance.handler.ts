...
import { FindLastAttendanceResult, FindLastEventResponse } from "@/temp/attendance/dto/attendance.type";
...
export class FindLastAttendanceHandler {
  ...
  async execute(query: FindLastAttendanceQuery): Promise<FindLastAttendanceResult | null> {
    ...
    const attendance = await this.prisma.attendanceSession.findFirst({
      where: {
        checkOut: null,
        OR: [{ Person: { id: responsableId } }, { EventInstance: { id: responsableId } }]
      },
      select: {
        id: true,
        modality: true,
    ...
    const { Person, EventInstance, EventAttendance, ...rest } = attendance;
    ...
    const attendanceFormatted = {
      ...
      support: {
        id: Person.id,
        fullName:
          `${Person.firstName} ${Person.lastName1}${Person.lastName2 ? " " + Person.lastName2 : ""}`.trim()
      }
    };
