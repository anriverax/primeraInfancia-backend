...
import { AttendanceEnum } from "prisma/generated/enums";
import { CreateAttendanceExceptionCommand } from "./cqrs/command/attendanceException/create-attendanceException.command";
import { GetAllAbsenceClassQuery } from "./cqrs/queries/absenceClassification/get-all-absenceClass.handler";
import { UpdateAttendanceSessionCommand } from "./cqrs/command/attendanceSession/update-attendanceSession.command";
import { UpdateEventAttendanceCommand } from "./cqrs/command/eventAttendance/update-eventAttendance.command";
...
export class AttendanceController {
  ...
  @AuthRequired()
  @Get("me/classification")
  async listClass(): Promise<GetAllEventResult[] | []> {
    const result = await this.queryBus.execute(new GetAllAbsenceClassQuery());
  ...
  @AuthRequired()
  @Get("me/finish/:id")
  async finish(@Req() req: Request, @Param("id") id: string): Promise<void> {
    const userId = req["user"].sub;
    await this.commandBus.execute(new UpdateAttendanceSessionCommand(parseInt(id)));
    await this.commandBus.execute(new UpdateEventAttendanceCommand(parseInt(id), userId));
  }
  /**
  ...
  async create(@Body() data: AttendanceDto, @Req() req: Request): Promise<NestResponse<void>> {
    ...
    for (const teacher of teacherId) {
      const eventAttendance = await this.commandBus.execute(
      ...
      if (rest.status === AttendanceEnum.AUSENTE && classificationId > 0) {
        await this.commandBus.execute(
          new CreateAttendanceExceptionCommand(
            {
              absenceClassificationId: classificationId,
              justificationFileUrl: justificationUrl,
              comment,
              eventAttendanceId: eventAttendance.id
            },
            userId
          )
        );
...
*/
  ...
  @AuthRequired()
  @Get("me/mentors")
  async listMyMentors(@Req() req: Request) {
    const userId = req["user"].sub;
